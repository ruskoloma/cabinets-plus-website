---
name: Calacatta Slim Gold
code: CalacattaSlimGold
slug: calacattaslimgold
countertopType: ""
inStock: true
storeCollection: in-stock
description: >-
  A soft white background is traced with very fine, elongated veining in pale gold and light gray, creating an airy,
  understated pattern. The movement is gentle and linear, with delicate diagonal lines that drift across the surface.
  Low contrast and restrained detailing give the slab a refined, minimalist, and elegant overall character.


  This sample allows you to inspect the authentic craftsmanship, durability, and finish before making your final
  selection.
picture: >-
  https://cabinetsplus4630.s3.us-west-2.amazonaws.com/uploads/countertops/calacattaslimgold/3650-calacatta-slim-gold-1.jpg
technicalDetails:
  - key: Thickness
    value: 3CM
    unit: ""
    order: 3
  - key: Store Collection
    value: in-stock
    unit: ""
    order: 6
media:
  - file: >-
      https://cabinetsplus4630.s3.us-west-2.amazonaws.com/uploads/countertops/calacattaslimgold/3650-calacatta-slim-gold-1.jpg
    kind: image
    mimeType: image/jpeg
    isPrimary: true
    label: CALACATTA SLIM GOLD 1.jpg
    altText: Calacatta Slim Gold
    description: ""
    sourceId: 3650
  - file: >-
      https://cabinetsplus4630.s3.us-west-2.amazonaws.com/uploads/countertops/calacattaslimgold/3671-calacatta-slim-gold.mp4
    kind: video
    mimeType: video/mp4
    isPrimary: false
    label: CALACATTA SLIM GOLD.mp4
    altText: ""
    description: ""
    sourceId: 3671
sourceId: 1724
sourceUpdatedAt: "2025-09-18T18:25:09.454Z"
---
