---
name: Calacatta Slim Grey
code: CalacattaSlimGrey
slug: calacattaslimgrey
countertopType: ""
inStock: true
storeCollection: in-stock
description: >-
  A soft white to very light gray background is traced with long, slim gray veining that drifts diagonally across the
  slab. The lines are delicate and airy, creating gentle movement rather than bold drama. Low contrast and restrained
  patterning give the surface a clean, refined, and quietly elegant design character.


  This sample allows you to inspect the authentic craftsmanship, durability, and finish before making your final
  selection.
picture: >-
  https://cabinetsplus4630.s3.us-west-2.amazonaws.com/uploads/countertops/calacattaslimgrey/3653-calacatta-slim-grey-1.jpg
technicalDetails:
  - key: Thickness
    value: 3CM
    unit: ""
    order: 3
  - key: Store Collection
    value: in-stock
    unit: ""
    order: 6
media:
  - file: >-
      https://cabinetsplus4630.s3.us-west-2.amazonaws.com/uploads/countertops/calacattaslimgrey/3653-calacatta-slim-grey-1.jpg
    kind: image
    mimeType: image/jpeg
    isPrimary: true
    label: CALACATTA SLIM GREY 1.jpg
    altText: Calacatta Slim Grey
    description: ""
    sourceId: 3653
  - file: >-
      https://cabinetsplus4630.s3.us-west-2.amazonaws.com/uploads/countertops/calacattaslimgrey/3672-calacatta-slim-grey.mp4
    kind: video
    mimeType: video/mp4
    isPrimary: false
    label: CALACATTA SLIM GREY.mp4
    altText: ""
    description: ""
    sourceId: 3672
sourceId: 1721
sourceUpdatedAt: "2025-09-18T18:25:09.704Z"
---
