import type {
  ...
  CatalogSystemInfo,
  CatalogVisualOption,
  FlooringOverviewItem,
  FlooringOverviewDataShape,
} from "./types";
...
const DEFAULT_FLOORING_OPTIONS: CatalogVisualOption[] = [
  { value: "lvp", label: "LVP", image: "/library/flooring/mozaik/waterwise-premium-portneuf-k2240rc.jpg" },
  { value: "laminate", label: "Laminate", image: "/library/flooring/lionsfloor/comfort-heights-city-pointe-li-ch08.jpg" },
  { value: "carpet", label: "Carpet", image: "/library/flooring/flooring2/natures-touch-fleece-nt-5.avif" },
  { value: "hardwood", label: "Hardwood", image: "/library/flooring/primafloors/engineered-hardwood-cesare-gw336.jpg" },
];
...
function normalizeCatalogSettings(value: unknown): CatalogSettingsData {
  ...
  return {
    ...
    rooms: Array.isArray(record?.rooms)
      ...
      : DEFAULT_ROOMS,
    countertopTypes: normalizeCatalogOptionList(record?.countertopTypes, []),
    flooringTypes: normalizeCatalogOptionList(record?.flooringTypes, DEFAULT_FLOORING_OPTIONS),
    _content_source: record?._content_source as unknown,
    _values: record?._values as unknown,
  };
}

function normalizeFlooring(value: unknown): FlooringOverviewItem | null {
  const record = asRecord(value);
  if (!record) return null;
...
export function normalizeFlooringOverviewQueryData(value: unknown): FlooringOverviewDataShape {
  const record = asRecord(value);
  const connection = asRecord(record?.flooringConnection);
  const edges = Array.isArray(connection?.edges) ? connection.edges : [];

  const normalizedEdges = edges
    .map((edge) => {
      const edgeRecord = asRecord(edge);
      const node = normalizeFlooring(edgeRecord?.node);
      if (!node || node.published !== true) return null;
      return { node };
    })
    .filter((edge): edge is { node: FlooringOverviewItem } => Boolean(edge));
...
export function getOverviewFlooringItems(data: FlooringOverviewDataShape): FlooringOverviewItem[] {
  const edges = Array.isArray(data.flooringConnection?.edges) ? data.flooringConnection.edges : [];

  return edges
    .map((edge) => edge?.node || null)
    .filter((node): node is FlooringOverviewItem => Boolean(node));
}
