...
interface FlooringProductInfoSectionProps {
...
function getGalleryTinaField(flooring: FlooringData, item: ProductGalleryItemViewModel): string | undefined {
  if (item.kind === "image" && flooring.picture?.trim() === item.file) {
    return tinaField(flooring as unknown as Record<string, unknown>, "picture") || undefined;
  }

  const matchedMedia = (flooring.media || []).find((media) => media?.file?.trim() === item.file);
  if (!matchedMedia) return undefined;

  return tinaField(matchedMedia as unknown as Record<string, unknown>, "file") || undefined;
}

function getGalleryFocusMediaItemId(flooring: FlooringData, item: ProductGalleryItemViewModel): string | undefined {
  if (item.kind === "image" && flooring.picture?.trim() === item.file) {
    return undefined;
  }
...
export default function FlooringProductInfoSection({
  flooring,
  pageText,
  block,
  previousProduct,
  ...
  galleryItems,
  technicalDetails,
}: FlooringProductInfoSectionProps) {
  ...
  const description = flooring.description?.trim() || "";
  const code = formatProductCode(flooring.code);
  ...
  const galleryItemsWithFields = useMemo(
    () =>
      galleryItems.map((item) => ({
        ...item,
        focusMediaItemId: getGalleryFocusMediaItemId(flooring, item),
        tinaField: getGalleryTinaField(flooring, item),
      })),
    [flooring, galleryItems],
  );

  return (
    <section className="bg-white" data-tina-field={edit ? undefined : flooringFieldName}>
      <div className="cp-container px-4 pb-12 pt-[34px] md:px-8 md:pb-16 md:pt-7">
        <div className="flex items-start justify-between gap-4 md:items-center">
          ...
          >
            <Link className="transition-colors hover:text-[var(--cp-primary-350)]" href="/flooring/catalog">
              <span data-tina-field={breadcrumbField}>
                {pageText.breadcrumbLabel}
              </span>
            </Link>
            <span>/</span>
            <span data-tina-field={tinaField(flooringRecord, "name") || undefined}>
              {displayName}
            </span>
          </nav>

          <div className="flex shrink-0 gap-2 md:gap-4">
            <ArrowNavButton ariaLabel="Previous product" direction="previous" href={previousProduct ? `/flooring/catalog/${previousProduct.slug}` : undefined} size="detail" />
            <ArrowNavButton ariaLabel="Next product" direction="next" href={nextProduct ? `/flooring/catalog/${nextProduct.slug}` : undefined} size="detail" />
          </div>
        </div>
          ...
          <div>
            {code ? (
              <p
                ...
                data-tina-field={tinaField(flooringRecord, "code") || undefined}
              >
                {code}
              </p>
            ) : null}
            ...
            <h1
              ...
              data-tina-field={tinaField(flooringRecord, "name") || undefined}
            >
              {displayName}
            </h1>
                ...
                <div className="order-3 max-w-[677px]">
                  ...
                  {descriptionOpen ? (
                    <p
                      ...
                      data-tina-field={tinaField(flooringRecord, "description") || undefined}
                    >
                      {description}
                    </p>
                  ) : null}
