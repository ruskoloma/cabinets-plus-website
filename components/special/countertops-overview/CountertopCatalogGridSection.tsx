...
interface CountertopCatalogGridSectionProps {
...
export default function CountertopCatalogGridSection({ block, data }: CountertopCatalogGridSectionProps) {
  const { edit } = useEditState();
  const router = useRouter();
  const pathname = usePathname();
  const currentPathname = pathname || "/countertops/catalog";
  ...
  const pageTitle = readString(block?.pageTitle, "Countertops");
