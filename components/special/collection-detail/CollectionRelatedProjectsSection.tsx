...
import { buildCollectionRelatedProjectCards } from "./helpers";
import type { CollectionOverviewItem } from "./types";
...
interface CollectionRelatedProjectsSectionProps {
  block?: Record<string, unknown> | null;
  collection: CollectionOverviewItem;
  overviewData: GalleryOverviewDataShape;
}

export default function CollectionRelatedProjectsSection({
  block,
  collection,
  overviewData,
}: CollectionRelatedProjectsSectionProps) {
  ...
  const titleText = (readStringField(block, "title") || "").trim() || "Related Projects";
  ...
  const relatedProjects = useMemo(
    () => buildCollectionRelatedProjectCards(collection, overviewData, tinaField),
    [collection, overviewData],
  );

  if (relatedProjects.length === 0) return null;

  // No data-tina-field on any ancestor of the cards: clicks rely entirely on the custom
  // postMessage channel so Tina focuses the current collection's `relatedProjects` list and
