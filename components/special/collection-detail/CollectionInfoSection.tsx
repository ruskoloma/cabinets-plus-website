...
import {
  ...
  getCollectionGalleryAlt,
  getCollectionHeading,
  getCollectionSlug,
} from "./helpers";
import type { CollectionGalleryItem, CollectionOverviewItem } from "./types";
...
interface CollectionInfoSectionProps {
  ...
  galleryItems: CollectionGalleryItem[];
}

export default function CollectionInfoSection({ block, collection, galleryItems }: CollectionInfoSectionProps) {
  const { edit } = useEditState();
  const quickEditEnabled = useTinaQuickEditEnabled();
  const rawCollection = collection as unknown as Record<string, unknown>;
  const collectionFieldName = tinaField(rawCollection) || undefined;
  const currentSlug = getCollectionSlug(collection, "collection");
  const heading = getCollectionHeading(collection, currentSlug);
  ...
  return (
    <>
      {/* No data-tina-field on outer <section>: clicks on media tiles must NOT bubble up to
          any ancestor so Tina focuses the collection's `media` list item via the custom postMessage
        ...
        <div className="cp-container px-4 pb-12 pt-[35px] md:px-8 md:pb-[88px] md:pt-[88px]">
          <div className="max-w-[1376px]">
            ...
            >
              ...
              <span>/</span>
              <span data-tina-field={tinaField(rawCollection, "title") || undefined}>{heading}</span>
            </nav>
            ...
            <div className="flex max-w-[1376px] flex-col gap-4 text-[var(--cp-primary-500)] md:gap-7">
              <h1
                ...
                data-tina-field={tinaField(rawCollection, "title") || undefined}
              >
                {heading}
              </h1>
              ...
              {description ? (
                <p
                  ...
                  data-tina-field={tinaField(rawCollection, "description") || undefined}
                >
                  {description}
                </p>
              ) : null}
            ...
            <div className="mt-8 grid grid-cols-2 gap-4 md:mt-8 md:grid-cols-4 md:gap-7">
              {galleryItems.map((item, index) => {
                ...
                return (
                    ...
                    onClick={(event) => {
                      if (useCustomMediaFocus) {
                        ...
                        focusTinaSidebarMediaItem({
                          rootFieldName: collectionFieldName,
                          mediaFile: item.file,
                        });
                        return;
                      }
                  ...
                  >
                    <FillImage
                      alt={getCollectionGalleryAlt(collection, item)}
        ...
        >
          ...
          <div className="max-h-[90vh] max-w-[min(95vw,1200px)]" onClick={(event) => event.stopPropagation()}>
            <div className="relative h-[min(85vh,900px)] w-[min(95vw,1100px)]">
              <FillImage
                alt={getCollectionGalleryAlt(collection, activeGalleryItem)}
