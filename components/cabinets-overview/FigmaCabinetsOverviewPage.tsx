...
const FALLBACK_MATERIAL_IMAGES = [
  "https://cabinetsplus4630.s3.us-west-2.amazonaws.com/library/home/product-cabinets.jpg",
...
export default function FigmaCabinetsOverviewPage({ page }: Props) {
    ...
    <div className="flex flex-col bg-white text-[var(--cp-primary-500)]">
          ...
          <div className="cp-container relative h-full px-4 md:px-8">
            <div className="absolute left-4 top-[247px] w-[345px] max-w-[calc(100%-32px)] md:left-8 md:top-auto md:w-auto md:max-w-[806px] md:bottom-8">
              ...
              >
                {text(
                  hero.subtext,
                  "Design the kitchen, bath, or laundry room you actually want. Work with our Spokane team on semi-custom cabinetry sized to your space, picked from door styles and finishes you can see in person at the showroom.",
                )}
              </p>
              <div className="mt-6 md:mt-8">
                <Button
                  dataTinaField={tinaField(heroRecord, "ctaLabel")}
                  href={text(hero.ctaLink, "/cabinets/catalog")}
                  variant="primary"
                >
      ...
      {hasTemplate("productsSection") ? (
        ...
        >
          ...
          >
            {text(materials.title, "Cabinet Styles")}
          </h2>
      ...
      {hasTemplate("processSection") ? (
        ...
        >
          <div className="cp-container px-4 md:px-[130px]">
            ...
            >
              {text(process.title, "Our Cabinet Process")}
            </h2>
            ...
            <div className="relative mx-auto mt-10 w-full max-w-[361px] md:mt-12 md:max-w-[1018px]">
              {processMobileLineSegments.slice(0, Math.max(0, processItems.length - 1)).map((segment, index) => (
                <span
                  ...
                  key={`cab-process-mobile-line-${index}`}
              ...
              {processDesktopLineSegments.slice(0, Math.max(0, processItems.length - 1)).map((segment, index) => (
                <span
                  ...
                  key={`cab-process-desktop-line-${index}`}
                  style={{ height: segment.height, left: segment.left, top: segment.top }}
                />
              ))}
            ...
            <div className="absolute left-4 top-[247px] w-[345px] max-w-[calc(100%-32px)] md:left-8 md:top-[225px] md:w-auto md:max-w-[806px]">
              ...
              >
                {text(
                  ...
                  "Swing by our Spokane showroom to open real cabinet doors, feel finishes under the lights, and ask our designers anything before you pick a configuration.",
                )}
              </p>
