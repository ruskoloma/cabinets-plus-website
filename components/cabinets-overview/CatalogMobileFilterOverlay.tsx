...
export default function CatalogMobileFilterOverlay({
  ...
  return (
    <div
      aria-label={title}
      aria-modal="true"
      className="fixed inset-0 z-[90] bg-white shadow-[0_8px_12px_6px_rgba(0,0,0,0.15),0_4px_4px_0_rgba(0,0,0,0.3)] md:hidden"
      role="dialog"
    >
      <button
      ...
      <div className="h-full overflow-y-auto px-4 pb-40 pt-12">
        ...
        {tabs ? <div className="mt-8 flex justify-center">{tabs}</div> : null}

        <div className={`mx-auto w-full ${tabs ? "mt-8 max-w-[361px]" : "mt-10 max-w-[362px]"}`}>{children}</div>
      </div>

      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-white/80 backdrop-blur-[8px]">
        <div className="flex h-full items-center justify-center">
          <Button className="pointer-events-auto !h-12 !px-8 !text-[20px]" onClick={onApply} size="small" variant="outline">
            Apply
          </Button>
        </div>
      </div>
