...
function getPageRecord(data: Record<string, unknown>) {
  const page =
    (data as { cabinetRefinishingMainPageSettings?: unknown }).cabinetRefinishingMainPageSettings ??
    (data as { page?: unknown }).page;

  return page && typeof page === "object" ? (page as Record<string, unknown>) : {};
}

function TinaCabinetRefinishingOverviewClient(props: Props) {
  const { data } = useTina({
    data: props.data,
    query: props.query || "",
...
export default function CabinetRefinishingOverviewClient(props: Props) {
  const hasLiveQuery = Boolean(props.query && props.query.trim().length > 0);
  const body = hasLiveQuery ? (
    <TinaCabinetRefinishingOverviewClient {...props} />
  ) : (
