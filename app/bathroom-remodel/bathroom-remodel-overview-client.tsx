...
function getPageRecord(data: Record<string, unknown>) {
  const page =
    (data as { bathroomRemodelMainPageSettings?: unknown }).bathroomRemodelMainPageSettings ??
    (data as { page?: unknown }).page;

  return page && typeof page === "object" ? (page as Record<string, unknown>) : {};
}

function TinaBathroomRemodelOverviewClient(props: Props) {
  const { data } = useTina({
    data: props.data,
    query: props.query || "",
...
export default function BathroomRemodelOverviewClient(props: Props) {
  ...
  return <TinaBathroomRemodelOverviewClient {...props} />;
}
