...
function getPageRecord(data: Record<string, unknown>) {
  const page =
    (data as { kitchenRemodelMainPageSettings?: unknown }).kitchenRemodelMainPageSettings ??
    (data as { page?: unknown }).page;

  return page && typeof page === "object" ? (page as Record<string, unknown>) : {};
}

function TinaKitchenRemodelOverviewClient(props: Props) {
  const { data } = useTina({
    data: props.data,
    query: props.query || "",
...
export default function KitchenRemodelOverviewClient(props: Props) {
  ...
  return <TinaKitchenRemodelOverviewClient {...props} />;
}
